...
- Comparison is the base probability of comparing the just-completed attempt with its immediate predecessor, after feedback. Define distance d = |previous a − current a| + |previous b − current b| + |previous observed answer − current observed answer| + 1 if the operation changes. The actual probability is comparison / max(1, d). This is a positional numeric distance: operand swaps are measured by their numeric changes, not treated as a special one-step transformation. It is an explicit toy assumption, not an empirically fitted metric. Discovery never searches older facts or sees future exercises. It also gates whether an already learned analogous solution is considered. Analogies need an available stored anchor and a learned rule. Execution slips occur independently per count/removal action in aggregate, perturbing the answer by one.
...
- Previously noticed pair evidence remains in the evidence archive even when accessibility weakens. Each attempted pair comparison costs max(1, d) comparison actions, including attempts that find no new pattern. For example, 2+3=5 → 2+4=6 has d=2; 2+3=5 → 2+7=9 has d=8, one quarter the probability and four times the effort when attempted. Identical equations have d=0, use the base probability and one action, and add no distinct evidence. The log records the previous/current attempt numbers, received answer, distance, adjusted probability, whether comparison occurred, evidence added, and newly acquired rules. Acquired rules become available on subsequent attempts subject to rule strength; taught rules are available from their lesson. Working memory, spontaneous procedural invention, elapsed-day spacing, learned strategy speeds, and time-calibrated costs are not implemented.
